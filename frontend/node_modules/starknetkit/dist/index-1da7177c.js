...
import { C as Z, P as H, a as y, b as R, U as E, r as $ } from "./connector-d812307b.js";
import { g as J } from "./trpc-94e013dd.js";
import { ArgentX as Y } from "./argentX.js";
import { A as O } from "./index-07fcd913.js";
const ee = {
...
var be = "[object Null]", pe = "[object Undefined]", W = k ? k.toStringTag : void 0;
function ve(r) {
  return r == null ? r === void 0 ? pe : be : W && W in Object(r) ? he(r) : we(r);
}
function Me(r) {
...
class s {
  ...
  static compareVersions(e, t, i = !1) {
    ...
    const l = s.map([e, t], (d) => {
      const h = a - s.getVersionPrecision(d), b = d + new Array(h + 1).join(".0");
      return s.map(b.split("."), (g) => new Array(20 - g.length).join("0") + g).reverse();
    });
...
const Ve = ({
  ...
  return (n ? r : [
    ...
    const h = C(d), b = S(d), g = e.find((w) => w.id === b?.id);
    if (g) {
      let w, p;
      h ? (w = d.icon, p = d.name) : (w = g.id === "argentX" ? { light: O, dark: O } : N(g.icon) ? { light: g.icon, dark: g.icon } : g.icon, p = g.id === "argentX" ? "Ready Wallet (formerly Argent)" : g.name);
      const B = t.find(
        (F) => F.id === (g.id === "argentMobile" ? "argentX" : b?.id)
      )?.downloads;
      return {
        name: p,
        id: g.id,
    ...
    if (v) {
      const { downloads: w } = v, p = v.id === "argentX" ? O : v.icon;
      return {
        name: v.name,
        id: v.id,
        icon: { light: p, dark: p },
    ...
    return !b || !b.id || !b.name ? null : G(d, t);
  }).filter((d) => d !== null);
};
class Re extends Z {
  constructor(t) {
  ...
  async ensureWallet(t) {
    const { getStarknetWindowObject: i } = await import("./index-13155a46.js"), { chainId: n, projectId: c, dappName: a, description: f, url: l, icons: d, rpcUrl: h } = this._options, b = re(), g = h ?? (!n || n === P.NetworkName.SN_MAIN ? b.mainnet : b.testnet), v = (await U.getDiscoveryWallets()).map((F) => F.id.toLowerCase() === "argentx" ? {
      ...F,
    ...
    await B.enable(), B.client.on("session_delete", () => {
      T(), this._wallet = null, $(), document.dispatchEvent(new Event("wallet_disconnected"));
    });
  }
}
class qe {
